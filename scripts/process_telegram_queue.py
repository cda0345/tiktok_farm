...
from scripts.create_gossip_post import (
    _fetch_article_data,
    _render_short,
    _send_video_to_telegram,
    _send_text_to_telegram,
)
...
def process_foto_request(request: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        print("📰 Buscando dados da matéria...")
        ...
        try:
            ...
            raise Exception(f"Erro ao buscar matéria: {e}")
        ...
        if not image_url:
            raise Exception("Não foi possível encontrar imagem na matéria")
        ...
        image_path = gossip_dir / "news_image.jpg"
        img_response = requests.get(image_url, headers=headers, timeout=30)
        with open(image_path, "wb") as f:
            f.write(img_response.content)
        
        # Cria arquivos de texto
        headline_file = gossip_dir / "headline.txt"
        with open(headline_file, "w", encoding="utf-8") as f:
            f.write(title[:100])  # Limita tamanho
        
        # Renderiza vídeo
        output_video = gossip_dir / "output" / f"post_{request['id']}.mp4"
